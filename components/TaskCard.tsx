...
export const TaskCard = ({ task }: TaskCardProps) => {
    ...
    return (
        <div onClick={() => setOpenCard((prev) => !prev)} ref={ref}
            ...
            {openCard && <div className="flex flex-col items-start px-4 w-full gap-1">
                <p className="text-neutral-400 text-sm md:text-base"> {task.description}</p>
                <p className="text-neutral-400 text-sm md:text-base">Due Date:{task.dueDate.toLocaleDateString()} </p>
            </div>}
        </div >
    );
};
